...
public final class StatefulContentfulClientProvider {
    ...
    public var currentLocaleCode: LocaleCode {
    ...
    where T: FieldKeysQueryable & EntryDecodable & Resource & StatefulResource {
        ...
        case .preview where stateMachine.state.editorialFeaturesEnabled == true:
            ...
            deliveryClient.fetchArray(of: T.self, matching: query) { [weak self] result in
    ...
    public func inferStateFromLinkedModuleDiffs<T>(statefulRootAndModules: (T, [LayoutSection]),
                                                   deliveryModules: [LayoutSection]) -> T where T: StatefulResource {
