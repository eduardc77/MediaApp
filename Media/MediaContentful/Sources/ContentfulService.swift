...
/// A class that acts as a service bus, bussing around contentful services down through the various components of the app.
public final class ContentfulService {
    
    public static let shared = ContentfulService()
    ...
    private init() {
        ...
        contentful = StatefulContentfulClientProvider(session: session,
                                                      ...
                                                      state: state)
        if let persistedLocaleCode = session.persistedLocaleCode(), let index = contentful.locales.map({ $0.code }).firstIndex(of: persistedLocaleCode) {
